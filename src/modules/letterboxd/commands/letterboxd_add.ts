...
import { buildFeedUrl, extractPosterUrl, extractReviewText, extractWatchedDate } from '../feeds.js';
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (latestItem) {
      ...
      if (ch?.isSendable()) {
        ...
        const watchedDate = extractWatchedDate(latestItem);
        const cleaned = {
          ...latestItem,
          contentSnippet: extractReviewText(latestItem) ?? undefined,
        };
        await ch.send({
          embeds: [buildItemEmbed(feedName, cleaned, true, thumbnailUrl, watchedDate)],
        });
        logger.info(
