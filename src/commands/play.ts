...
import { YtDlpError, resolveTrack } from '../audio/ytdlp.js';
...
const command: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setName('query')
        .setDescription('A URL (YouTube, SoundCloud, etc.) or search terms.')
        .setRequired(true),
    ),
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      const message =
        err instanceof YtDlpError ? err.message : 'Failed to resolve track.';
      logger.error('resolveTrack failed:', err);
